...
export const DrawnCardsHistoryView: React.FC<DrawnCardsHistoryViewProps> = ({ 
  ...
  const renderEmptyState = () => (
    <div className="w-full max-w-sm sm:max-w-md perspective mx-auto relative" style={{ height: 'auto' }}>
      <div style={{ paddingTop: `${(7 / 5) * 100}%` }} className="relative">
        <div className="absolute inset-0 bg-slate-800/50 border-2 border-dashed border-slate-700 rounded-xl shadow-xl flex flex-col items-center justify-center p-4 text-center shimmer-effect overflow-hidden">
          <CornerGlyphGrid position="top-left" glyphColorClass="text-white/70" glyphSizeClass="text-lg sm:text-xl md:text-2xl" gridGapClass="gap-1 sm:gap-1.5"/>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-auto pt-2 relative z-10">Select a theme from the top to draw your first card.</p>
          <CornerGlyphGrid position="bottom-right" glyphColorClass="text-white/70" glyphSizeClass="text-lg sm:text-xl md:text-2xl" gridGapClass="gap-1 sm:gap-1.5"/>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full max-w-6xl px-1 sm:px-2 flex flex-col items-center">
      <div className="w-full flex flex-row justify-center items-start gap-2 sm:gap-4 mb-4 sm:mb-6">
        ...
        {isMainCardAreaActive ? (
           <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 mt-1 sm:mt-2">
             <button 
                onClick={onOpenVoiceSettings}
                className="p-2.5 sm:p-3 rounded-full bg-slate-700 hover:bg-sky-600 text-slate-300 hover:text-white transition-colors duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-slate-900"
        ...
        ) : ( <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12"></div> )}
      </div>
      
      {olderCards.length > 0 && (
        <div className="w-full grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-2 sm:gap-3 md:gap-4 pb-24">
