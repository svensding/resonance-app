...
export const BottomToolbar: React.FC<BottomToolbarProps> = ({ 
  ...
  return (
    <div className="px-2 py-1.5 sm:px-4 sm:py-2 flex items-center space-x-2 sm:space-x-3 w-full overflow-x-auto scrollbar-thin">
      {/* Group Setting Button */}
      <button
        onClick={onOpenGroupSettingModal}
        disabled={disabled}
        className={`px-3 py-1.5 text-xs sm:text-sm rounded-md transition-all duration-150 ease-in-out focus:outline-none whitespace-nowrap flex-shrink-0
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="inline-block h-3.5 w-3.5 mr-1.5 -ml-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
        </svg>
        Setting: {currentGroupSettingLabel}
      </button>

      {/* Separator */}
      <div className="h-5 w-px bg-slate-600/70 flex-shrink-0"></div>


      {/* Participant Management */}
      <div className="flex items-center space-x-1 sm:space-x-2 flex-nowrap">
        {participants.map((participant) => (
          <div key={participant.id} className="flex-shrink-0 flex items-center space-x-1">
            {editingNameId === participant.id ? (
              <input
                ...
                className={`px-2.5 py-1.5 text-xs sm:text-sm bg-slate-700 border border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-white w-28 sm:w-32 font-normal ${disabled ? 'cursor-not-allowed opacity-70' : ''}`}
                autoFocus
                disabled={disabled}
              ...
              <button
                ...
                disabled={disabled}
                className={`px-2.5 py-1.5 text-xs sm:text-sm rounded-md transition-colors duration-150
                            ${activeParticipantId === participant.id ? 'bg-sky-500 text-white font-semibold ring-2 ring-sky-300 ring-offset-1 ring-offset-slate-800' : 'bg-slate-600 hover:bg-slate-500 text-slate-200 font-normal'}
                            truncate w-28 sm:w-32 ${disabled ? 'cursor-not-allowed opacity-70' : 'hover:bg-slate-500'}`}
            ...
            { (participants.length > 0) && 
                ...
                disabled={disabled}
                className={`p-1 rounded-full text-slate-400 transition-colors duration-150 flex-shrink-0
      ...
      <button
        ...
        className={`p-1.5 sm:p-2 rounded-full bg-sky-600 text-white transition-colors duration-150 ml-auto flex-shrink-0
