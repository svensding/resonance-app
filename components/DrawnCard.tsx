...
const DrawnCardComponent: React.FC<DrawnCardProps> = ({
  ...
  const glyphSize = isNewest ? "text-lg sm:text-xl md:text-2xl" : "text-base sm:text-lg";
  const glyphGap = isNewest ? "gap-1 sm:gap-1.5" : "gap-0.5 sm:gap-1";
  ...
  const renderCardBackNotes = () => {
    if (!parsedGuidance.length) return <p className="text-base text-slate-400 font-atkinson-regular text-center">No additional guidance for this card.</p>;
    return (
      <div className="space-y-3">
        {parsedGuidance.map((section, index) => (
          section.content.trim() ? (
            <div key={index}>
              <h6 className="font-semibold text-base text-slate-300 mb-0.5">{section.heading}</h6>
              <p className="text-base text-slate-200 font-atkinson-regular whitespace-pre-wrap">{section.content}</p>
            </div>
          ) : null
        ))}
      </div>
  ...
  if (isLoadingPlaceholder) {
    const baseWidthClass = isNewest ? "w-full max-w-sm sm:max-w-md" : "w-full max-w-xs";
        ...
        <div style={{ paddingTop: `${CARD_ASPECT_RATIO_MULTIPLIER * 100}%` }} className="relative">
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center flex-grow relative z-10"> 
              <p className={`text-3xl sm:text-4xl font-semibold font-playfair text-white/70 brand-text-container`}><span className="brand-text-r">R</span>esonance <span className="font-normal">⦾</span></p>
              <p className="text-xs sm:text-sm text-slate-300/80 mt-3 relative z-10">{loadingText}</p>
            </div>
  ...
  const baseWidthClass = isNewest ? "w-full max-w-sm sm:max-w-md" : "w-full max-w-[180px] xs:max-w-[200px] sm:max-w-xs"; 
  const themeNameSizeClasses = isNewest ? "text-xs sm:text-sm" : "text-[0.65rem]";
  const participantNameSizeClasses = isNewest ? "text-[0.65rem] sm:text-xs" : "text-[0.6rem]";
  const utilityButtonIconSize = isNewest ? "h-5 w-5 sm:h-6 sm:w-6" : "h-4 w-4";
  const utilityButtonPadding = isNewest ? "p-1.5 sm:p-2" : "p-1";
  const utilityButtonRotateIconFontSize = isNewest ? '1rem' : '0.8rem';
  const cardPaddingClass = isNewest ? "p-3 sm:p-4" : "p-2 sm:p-3"; 
  const promptTextHorizontalPadding = "px-6 sm:px-7"; 
  
  const promptTextStyle: React.CSSProperties = { textWrap: 'balance' as any, lineHeight: '1.25' };
  ...
  const brandingTextSize = isNewest ? "text-[0.65rem] sm:text-xs" : "text-[0.55rem] sm:text-[0.6rem]";
  const actionButtonClasses = `p-1.5 sm:p-2 rounded-full transition-all duration-200 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-within:opacity-100`;
  const actionButtonIconSize = isNewest ? "h-5 w-5 sm:h-6 sm:w-6" : "h-4 w-4";

  let promptTextSizeClasses = "";
  if (isNewest && !showCardBackView && !showLLMDetailsView) {
    promptTextSizeClasses = (promptText && promptText.length > LONG_PROMPT_THRESHOLD) ? "text-2xl md:text-3xl" : "text-3xl md:text-4xl";
  } else if (!isNewest) { 
    promptTextSizeClasses = (promptText && promptText.length > LONG_PROMPT_THRESHOLD) ? "text-lg" : "text-xl";
  }
    ...
    >
      <div style={{ paddingTop: `${CARD_ASPECT_RATIO_MULTIPLIER * 100}%` }} className="relative">
        <div className={`absolute inset-0 preserve-3d transition-transform duration-700 ease-in-out ${isRevealed ? 'rotate-y-180' : ''}`}>
          ...
          <div className={`absolute w-full h-full backface-hidden ${overlayBaseClasses} ${overlayDashedBorderClasses} rounded-xl shadow-xl flex flex-col items-center justify-center p-4 text-center overflow-hidden`}>
            <CornerGlyphGrid position="top-left" glyphColorClass={glyphColor} glyphSizeClass={glyphSize} gridGapClass={glyphGap} />
            <div className="text-6xl text-sky-500/50 font-playfair" aria-hidden="true">⦾</div>
            <p className="text-xl font-semibold text-white/70 mt-2 font-playfair brand-text-container"><span className="brand-text-r">R</span>esonance</p> 
          ...
          <div className={`absolute w-full h-full backface-hidden rotate-y-180 
            ...
            {isRevealed && llmPromptForTextGeneration && !showCardBackView && !showLLMDetailsView && (
              <button onClick={() => { setShowLLMDetailsView(true); setShowCardBackView(false); setLlmDetailsTab('input'); }} className={`absolute top-2 right-2 sm:top-3 sm:right-3 ${utilityButtonPadding} rounded-full bg-black/20 hover:bg-black/40 text-slate-300 hover:text-white transition-colors duration-200 z-30 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-within:opacity-100`} aria-label="Show LLM Details" title="Show LLM Details">
            ...
            <div className={`relative z-10 flex flex-col flex-grow h-full ${cardPaddingClass}`}>
              ...
              <div className={`flex-grow overflow-y-auto hide-scrollbar scrollbar-thumb-white/40 scrollbar-track-transparent my-1 flex flex-col w-full
                  ${showLLMDetailsView ? 'items-stretch justify-start' : showCardBackView ? 'items-center justify-start pt-1 sm:pt-2' : 'items-center justify-center pt-5 pb-5'}`}>
                {showLLMDetailsView ? (
                  <div className="flex flex-col h-full w-full">
                     <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                        <div className="flex space-x-1.5 sm:space-x-2">
                          <button onClick={() => setLlmDetailsTab('input')} className={`px-2.5 py-1 sm:px-3 text-xs sm:text-sm rounded-md ${llmDetailsTab === 'input' ? 'bg-sky-600 text-white font-semibold' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>Input Prompt</button>
                          <button onClick={() => setLlmDetailsTab('output')} className={`px-2.5 py-1 sm:px-3 text-xs sm:text-sm rounded-md ${llmDetailsTab === 'output' ? 'bg-sky-600 text-white font-semibold' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>Raw Output</button>
                        </div>
                      ...
                      <div className={`overflow-y-auto text-slate-300 bg-slate-900/60 p-2 sm:p-3 rounded-md scrollbar-thin flex-grow ${isNewest ? 'text-xs' : 'text-[0.65rem] sm:text-xs'}`}>
                  ...
                  <div className="flex flex-col flex-grow w-full text-center">
                    ...
                    <div className={`flex-grow text-left overflow-y-auto scrollbar-thin pr-1 font-atkinson-regular`}>{renderCardBackNotes()}</div>
                  </div>
                ) : (
                  <>
                    <div className="mb-1 sm:mb-1.5 w-full text-center">
              ...
              {!showLLMDetailsView && (
                <div className="relative z-[1] flex justify-between items-center pt-1 sm:pt-1.5 mt-auto w-full">
                  ...
                  <div className="flex items-center space-x-1 sm:space-x-1.5">
                    <button onClick={() => onDislike(id)} className={`${actionButtonClasses} ${feedback === 'disliked' ? 'bg-sky-700/90 text-white scale-110 ring-1 ring-sky-500' : 'bg-black/30 hover:bg-slate-600/70 text-slate-300 hover:text-white'}`} aria-label="Dislike" title="Dislike">
                    ...
                    <button onClick={() => onPlayAudioForMainPrompt({ text: promptText, audioData, audioMimeType })} disabled={(!promptText && !audioData) || isLoadingPlaceholder || (promptText && promptText.startsWith("The Resonance seems to be quiet"))} className={`${actionButtonClasses} bg-black/30 hover:bg-sky-600/80 text-slate-300 hover:text-white disabled:opacity-50 disabled:hover:bg-black/30`} aria-label="Play Audio" title="Play Audio">
                      <svg xmlns="http://www.w3.org/2000/svg" className={actionButtonIconSize} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </button>
                    <button onClick={() => onLike(id)} className={`${actionButtonClasses} ${feedback === 'liked' ? 'bg-sky-700/90 text-white scale-110 ring-1 ring-sky-500' : 'bg-black/30 hover:bg-slate-600/70 text-slate-300 hover:text-white'}`} aria-label="Like" title="Like">
