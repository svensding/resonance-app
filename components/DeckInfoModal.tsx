    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h2 id="deck-info-title" className="text-xl sm:text-2xl font-semibold text-sky-400 font-playfair">{item.name}</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="space-y-3 text-slate-300 max-h-[60vh] overflow-y-auto scrollbar-thin pr-2">
          <p className="text-sm sm:text-base whitespace-pre-wrap">{mainDescription}</p>
          
          {inspiredByText && (
            <div className="mt-3 pt-3 border-t border-slate-700">
              <h4 className="text-sm font-semibold text-sky-300 mb-1">Inspired by:</h4>
              <p className="text-sm sm:text-base whitespace-pre-wrap">{inspiredByText}</p>
            </div>
          )}

          {/* If it's a DeckSet and we want to list its micro-decks (future enhancement)
          {'microDeckIds' in item && Array.isArray(item.microDeckIds) && item.microDeckIds.length > 0 && (
            <div className="mt-3 pt-3 border-t border-slate-700">
              <h4 className="text-sm font-semibold text-sky-300 mb-1">Contains themes like:</h4>
              <ul className="list-disc list-inside text-sm space-y-0.5">
                {item.microDeckIds.map(id => <li key={id}>{getMicroDeckById(id)?.internal_name || id}</li>)}
              </ul>
            </div>
          )}
          */}
